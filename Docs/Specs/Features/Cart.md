...
since Q37, that the PDP stepper fires them per step.

### Event: `add_to_bag`

**When**: after the cart mutation **succeeds** — not on tap. Includes each successful `+` on the PDP
stepper (Q37).
**Parameters**: `product_id: String`

### Event: `remove_from_bag`

**When**: after `removeFromCart` **succeeds**, and after each successful `−` on the PDP stepper —
including a step that only lowers the quantity (e.g. 3 → 2) and leaves the line in place (Q37).
**Parameters**: `product_id: String`
...
| Q20 | ~~Debounce each stepper, disable controls in flight, re-fetch on failure.~~ **Superseded by Q27**, then **reinstated without the debounce by Q37**: controls disable in flight and the cart is re-read on failure. With the controls disabled a tap burst cannot become concurrent round-trips, so the debounce buys nothing. | `updateCart` takes the full `lines` array, so two in-flight updates clobber each other. Pessimistic writes (Q8) supply the disable; the debounce stops a tap burst becoming three round-trips. The re-fetch is required because BigCommerce's `updateCart` loops line-by-line and can fail mid-loop with earlier lines committed. |
...
| Q37 | **The PDP gets a quantity stepper.** Once the selected variant is in the bag (and, where the product offers a size choice, a size is picked), Add to bag becomes `− n +`. `+` is capped at `min(stock, 100)`; `−` at one removes the line through `removeFromCart`, putting Add to bag back. Writes go through `updateCart` with the whole `lines` array, queued behind every other cart operation. Every failed write re-reads the cart before surfacing "Couldn't update quantity". Each successful step fires `add_to_bag` / `remove_from_bag`. | Queuing stops an update overtaking an add and dropping its line. The re-read covers BigCommerce applying `updateCart` line-by-line and a timeout after the server committed. Per-step events keep the event stream meaning "a unit went in / came out", consistent with re-tapping Add to bag; the schema changes stay deferred (Q29). |
