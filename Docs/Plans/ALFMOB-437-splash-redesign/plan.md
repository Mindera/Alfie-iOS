...
2. **Spinner → DS `ThemedSpinnerView` (SharedUI).** Renders the Figma Loading Spinner artwork
...
- [x] `ThemedSpinnerView` — rotates the exported Loading Spinner arc image (`ThemedImage.loadingSpinner`), sizes S/M/L, with `#Preview`.
...
| `SharedUI/Theme/Components/Loader/ThemedSpinnerView.swift` | SharedUI | add | animated DS loading spinner (rotates the arc image, S/M/L) |
