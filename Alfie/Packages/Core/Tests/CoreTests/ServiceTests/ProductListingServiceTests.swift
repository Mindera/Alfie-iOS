...
final class ProductListingServiceTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        sut = .init(productService: ProductService(bffClient: mockClientService), configuration: paginationConfiguration)
    }
    ...
    func test_first_page_calls_bff_service_with_params() {
        ...
        mockClientService.onProductListingCalled = { offset, limit, categoryId, query in
        ...
        Task {
            _ = try await sut.paged(categoryId: "category id", query: "query")
        }
    ...
    func test_pagination_info_provides_next_page() {
        let expectation = expectation(description: "Wait for service call")
        mockClientService.onProductListingCalled = { _, _, _, _ in
            expectation.fulfill()
    ...
    func test_next_page_calls_bff_service_while_has_next_pages() async {
        ...
        mockClientService.onProductListingCalled = { offset, _, _, _ in
            expectation.fulfill()
        ...
        do {
            ...
            _ = try await sut.paged(categoryId: "try 1", query: "query 1")
            XCTAssertTrue(sut.hasNext())
            // second page successfully but no next page
            _ = try await sut.next(categoryId: "try 2", query: "query 2")
            XCTAssertFalse(sut.hasNext())
            // third page failure
            _ = try await sut.next(categoryId: "try 3", query: "query 3")
        } catch {
            guard case let bffError = error as? BFFRequestError,
                  case .product(let productError) = bffError?.type,
                  case .noProducts(let category, let query) = productError
            else {
