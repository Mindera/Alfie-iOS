...
struct ProductCarouselDemoView: View {
    var body: some View {
        VStack(spacing: Spacing.space0) {
            ...
            ProductCarouselView(data: Product.fixtures, header: {
            ...
            ProductCarouselView(data: Array(Product.fixtures.prefix(2)), header: {
                ProductCarouselHeader(title: "New In",
                                      subtitle: "Get a latest in fashion")
            }, content: { product in
                productCard(product)
    ...
    @ViewBuilder private func productCard(_ product: Product) -> some View {
