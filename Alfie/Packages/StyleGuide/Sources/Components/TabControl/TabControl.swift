...
public struct TabControl: View {
    ...
    private var tabControlItemsView: some View {
        HStack(alignment: .center, spacing: configuration.itemSpacing) {
            ...
            ForEach(Array(options.enumerated()), id: \.0) { index, value in
                Button {
                    ...
                    .foregroundStyle(currentIndex == index ? Constants.selectedItemFontColor : Constants.unselectedItemFontColor)
                    .overlay {
                        if currentIndex == index {
                            ...
                            .matchedGeometryEffect(id: Constants.matchedGeometryEffectID,
                                                   in: namespace)
                ...
                .`if`(currentIndex == index, whenTrue: { tab in
                    tab.accessibilityAddTraits(.isSelected)
                }, whenFalse: { tab in
                    tab.accessibilityRemoveTraits(.isSelected)
                })
            }
        ...
        .background {
            ...
            .frame(height: Constants.itemHeight)
            .frame(minWidth: configuration.isIntrisicSize ? UIScreen.main.bounds.width * 2 : 0) // this way even with the elastic / bounce effect of the scrollView, the line will always be there
        }
