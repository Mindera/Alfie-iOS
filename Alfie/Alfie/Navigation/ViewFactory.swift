...
final class ViewFactory: ViewFactoryProtocol {
    ...
    private weak var serviceProvider: ServiceProviderProtocol!
        ...
        switch screen {
            case .tab(let tab):
                switch tab {
                    ...
                    case .shop:
                        ShopView(categoriesViewModel: CategoriesViewModel(navigationService: serviceProvider.navigationService, showToolbar: false),
                                 brandsViewModel: BrandsViewModel(brandsService: serviceProvider.brandsService),
                                 servicesViewModel: WebViewModel(webFeature: .storeServices,
                                                                 dependencies: WebDependencyContainer(deepLinkService: serviceProvider.deepLinkService,
                                                                                                      webViewConfigurationService: serviceProvider.webViewConfigurationService,
                                                                                                      webUrlProvider: serviceProvider.webUrlProvider)))
                    case .wishlist:
                        WishListView()
                    case .bag:
                        BagView(viewModel: BagViewModel(dependencies: BagDependencyContainer(deepLinkService: serviceProvider.deepLinkService,
                                                                                             webUrlProvider: serviceProvider.webUrlProvider,
                                                                                             webViewConfigurationService: serviceProvider.webViewConfigurationService)))
                }
            case .account:
            ...
            case .search(let transition):
                let dependencies = SearchDependencyContainer(recentsService: serviceProvider.recentsService,
            ...
            case .productListing(let configuration):
                ProductListingView(viewModel: ProductListingViewModel(productListingService: ProductListingService(productService: serviceProvider.productService,
            ...
            case .webFeature(let feature):
                WebView(viewModel: WebViewModel(webFeature: feature,
                                                dependencies: WebDependencyContainer(deepLinkService: serviceProvider.deepLinkService,
                                                                                     webViewConfigurationService: serviceProvider.webViewConfigurationService,
                                                                                     webUrlProvider: serviceProvider.webUrlProvider)))
                    .withToolbar(for: screen)
            ...
            case .productDetails(let type):
                switch type {
                    case .id(let id):
                        ProductDetailsView(viewModel: ProductDetailsViewModel(productId: id,
                                                                              product: nil,
                                                                              dependencies: ProductDetailsDependencyContainer(productService: serviceProvider.productService,
                                                                                                                              webUrlProvider: serviceProvider.webUrlProvider)))
                    case .product(let product):
                        ProductDetailsView(viewModel: ProductDetailsViewModel(productId: product.id,
                                                                              product: product,
                                                                              dependencies: ProductDetailsDependencyContainer(productService: serviceProvider.productService,
                                                                                                                              webUrlProvider: serviceProvider.webUrlProvider)))
                }
