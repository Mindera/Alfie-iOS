...
///
/// Every tab with a Search bar builds the scanner from the same decisions — where a recognised link
/// goes, the one recovery a refused camera has is the door out to Settings, and closing clears the
/// tab's overlay. Held here rather than in each flow so that a second tab gaining a Scan control is
/// one call, not another copy of them.
///
