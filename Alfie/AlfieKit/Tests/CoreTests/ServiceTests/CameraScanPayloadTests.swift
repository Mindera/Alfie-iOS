...
final class CameraScanPayloadTests: XCTestCase {
    ...
    /// A Selfridges price tag prints its GTIN-13 in a Code 128 symbol, not an EAN-13 one — the
    /// `2600030000445` on a Cairo Dress tag reads as Code 128 and would otherwise never be seen.
    func test_code128_is_read_as_a_barcode() {
        XCTAssertEqual(CameraScanService.payload(for: .code128, value: "2600030000445"), .code128("2600030000445"))
    }
    ...
    func test_other_symbologies_are_ignored() {
        for symbology in [VNBarcodeSymbology.ean8, .pdf417, .aztec] {
    ...
    func test_a_code_the_scanner_could_not_read_is_ignored() {
        ...
        XCTAssertNil(CameraScanService.payload(for: .code128, value: nil))
    }
}
